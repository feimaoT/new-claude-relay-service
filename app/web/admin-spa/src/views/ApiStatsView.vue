...
import WechatQrcodeModal from '@/components/common/WechatQrcodeModal.vue'
...
// 二维码图片路径 - 支持 png 或 jpg 格式
const wechatQrcodeUrl = ref('')

// 动态导入二维码图片（支持 png 和 jpg）
const loadQrcodeImage = () => {
  // 使用 import.meta.glob 进行可选导入，避免构建时报错
  const images = import.meta.glob('@/assets/images/wechat-qrcode.{jpg,png}', { eager: true })
  const imagePath = Object.keys(images)[0]
  if (imagePath && images[imagePath]) {
    wechatQrcodeUrl.value = images[imagePath].default
  }
}
